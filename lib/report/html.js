        ...
        if (sumCount > 0) { //only highlight if partial branches are missing
            for (i = 0; i < branchArray.length; i += 1) {
                ...
                if (count === 0) { //skip branches taken
                    ...
                    if (branchMeta[branchName].type === 'if' && text && text.insertAt) { // and 'if' is a special case since the else branch might not be visible, being non-existent
 ...
 * -----
 *
 *      var report = require('@thetrg/istanbul').Report.create('html');
 *
 *
 * @class HtmlReport
 * @extends Report
